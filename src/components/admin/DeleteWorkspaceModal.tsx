...
export function DeleteWorkspaceModal({
  ...
  const handleDelete = async (e: React.FormEvent) => {
    ...
    setIsDeleting(true);
    try {
      // 1. Delete conversations and their messages subcollections
      ...
      await Promise.all([
        invalidateCache(cacheKeys.userWorkspaces(ownerId)),
        invalidateCache(cacheKeys.workspace(workspaceId)),
        invalidateCache(cacheKeys.workspaceStats(workspaceId)),
      ]);
  ...
  return (
    <div className="fixed inset-0 bg-gray-900/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
      <div className="bg-[var(--bg-card)] border border-[var(--border-color)] rounded-[40px] p-10 w-full max-w-lg shadow-2xl relative overflow-hidden animate-in zoom-in-95 duration-300">
        ...
        <div className="mb-8 text-center">
          ...
          <h3 className="text-3xl font-black text-[var(--text-main)] tracking-tight mb-2">
            Delete Workspace?
          </h3>
