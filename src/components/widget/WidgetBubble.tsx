...
export function WidgetBubble({ isOpen, onClick, color, unreadCount = 0 }: { isOpen: boolean, onClick: () => void, color: string, unreadCount?: number }) {
  ...
  return (
    <button 
      onClick={onClick}
      className="w-14 h-14 shadow-2xl flex items-center justify-center text-white pointer-events-auto transition-all transform hover:scale-110 active:scale-95 z-[99999] group overflow-hidden rounded-full"
