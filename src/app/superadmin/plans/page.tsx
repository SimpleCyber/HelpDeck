  ...
  const PlanCard = ({ 
    ...
    return (
      <div className="bg-white rounded-2xl border border-slate-200 shadow-sm overflow-hidden flex flex-col h-full">
        {/* Card Header */}
        <div className={cn("px-6 py-4 border-b border-slate-100", colorClass)}>
          <h3 className="font-bold text-slate-800">{title}</h3>
          <p className="text-xs text-slate-500 font-medium mt-0.5 uppercase tracking-wide">Configuration</p>
        </div>
        ...
        <div className="p-6 space-y-6 flex-1">
          <div className="space-y-4">
             <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Workspaces Limit</label>
                <div className="relative">
                  <input
                    type="number"
                    value={plan.maxWorkspaces}
                    onChange={(e) => handleChange(planKey, "maxWorkspaces", parseInt(e.target.value) || 0)}
                    className="w-full pl-4 pr-12 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all"
                  />
             ...
             <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Members / Workspace</label>
                <input
                  type="number"
                  value={plan.maxMembersPerWorkspace}
                  onChange={(e) => handleChange(planKey, "maxMembersPerWorkspace", parseInt(e.target.value) || 0)}
                  className="w-full px-4 py-2.5 bg-slate-50 border border-slate-200 rounded-xl text-slate-900 font-semibold focus:outline-none focus:ring-2 focus:ring-blue-500 focus:bg-white transition-all"
                />
             </div>

             <div className="space-y-1">
                <label className="text-xs font-bold text-slate-700 uppercase tracking-wider">Customers Limit</label>
                <input
                  type="number"
          ...
          <div className="pt-4 mt-auto border-t border-slate-100 flex items-center justify-between">
             <div className="text-sm font-bold text-slate-700">Image Uploads</div>
             <button
              ...
              className={cn(
                ...
                plan.allowImageUpload ? "bg-emerald-500" : "bg-slate-200"
              )}
            >
              <div className={cn(
      ...
      <div className="max-w-7xl mx-auto space-y-8">
        <div className="flex items-center justify-between">
          <div className="space-y-1">
            ...
            <p className="text-sm text-slate-500">Manage feature limits for each subscription tier.</p>
          </div>
          ...
          <button
            ...
            disabled={saving}
            className={cn(
               "flex items-center gap-2 px-6 py-2.5 rounded-xl font-bold text-white shadow-lg shadow-blue-500/20 active:scale-95 transition-all text-sm",
