...
export default function SuperAdminOverview() {
  ...
  const handleAction = async (req: UpgradeRequest, action: 'approve' | 'reject') => {
    ...
    <main className="flex-1 overflow-y-auto bg-slate-50/50 dark:bg-[#0f172a] p-8 h-full">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        <div className="flex flex-col gap-1">
          ...
          <p className="text-slate-500 dark:text-slate-400 text-sm">Welcome back to the command center.</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
          ...
          <div className="bg-white dark:bg-[#1e293b] rounded-2xl p-6 border border-slate-100 dark:border-slate-700 shadow-sm flex flex-col">
             ...
             <div className="flex-1 space-y-4">
               {loading ? (
                 ...
                 requests.map(req => (
                   <div key={req.id} className="p-4 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-100 dark:border-slate-700 flex items-center justify-between group hover:border-blue-200 dark:hover:border-blue-800 transition-colors">
                      <div className="min-w-0">
                         <div className="font-bold text-slate-900 dark:text-white truncate">{req.userEmail}</div>
                         <div className="text-xs text-slate-500 dark:text-slate-400 flex items-center gap-2 mt-0.5">
                           <span className="capitalize font-semibold text-blue-600 dark:text-blue-400">{req.plan} Plan</span>
                           <span>•</span>
          ...
          <div className="bg-white dark:bg-[#1e293b] rounded-2xl p-6 border border-slate-100 dark:border-slate-700 shadow-sm">
             <div className="flex items-center justify-between mb-6">
               ...
               <span className="text-xs font-medium text-slate-500 dark:text-slate-400">Last 24h</span>
             </div>
